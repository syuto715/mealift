...
import { parseZenshoPdf, extractSizeRows, groupSizeRows } from '../parsers/zensho';
import { SUKIYA_MENU_NAMES } from '../parsers/menu_names/sukiya';
import { NAKAU_MENU_NAMES } from '../parsers/menu_names/nakau';

function probe(label: string, txtPath: string, menuNames: string[]) {
  const text = fs.readFileSync(txtPath, 'utf-8');
  const rows = extractSizeRows(text);
...
if (nakauTxt) probe('nakau', nakauTxt, NAKAU_MENU_NAMES);
