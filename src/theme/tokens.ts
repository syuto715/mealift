export const colors = {
  ...
  // ⚠️ 使い分けの契約: statusX (bright fill) は情報を担わない tint 背景
  // (+alpha) と装飾面専用 — light 面では非テキスト 3:1 を満たさない
  // (success 2.22 / warning 2.20 on white)。状態を担うリング・アイコン・
  // バー・小テキストには必ず statusXText を使う (light=濃色で 3:1/4.5:1
  // とも pass、dark=bright mapback)。
