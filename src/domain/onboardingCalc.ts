...
// which the user has advanced past protein-target. By this point,
// every required calc input is user-set (gender / birthYear /
// heightCm / currentWeightKg from [3], activityLevel from [4],
// targetWeightKg / weeklyRatePct from [5], mealPlan from [7],
// mealTimings from [8], proteinFactor from [9]). Used by:
//   - calculateAll's trust-boundary guard (Phase A-4 Codex Important #1)
//   - onboardingService's per-field gating threshold (Phase A-5)
//
// Codex pass 1 / Phase D-2 sign-off fix — bumped from 8 to 9
// because goal-summary (step 6, read-only) shifts every input
// step by +1 relative to the original A-5 numbering. See the
// matching adjustment to FIELD_STEP_THRESHOLDS in
// onboardingService.ts.
export const ONBOARDING_STEP_FULL_INPUT = 9;
